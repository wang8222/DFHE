...
def parse_args():
    ...
    parser.add_argument('--data_name', nargs='?', default='douban_movie',
    ...
    parser.add_argument('--cf_batch_size', type=int, default=20000,
                        help='CF batch size.')
    parser.add_argument('--kg_batch_size', type=int, default=2000,
                        help='KG batch size.')
    parser.add_argument('--nd_batch_size', type=int, default=1500,
    ...
    parser.add_argument('--test_batch_size', type=int, default=12000,
    ...
    parser.add_argument('--mpset',
                        default="[[['2', '3','8','1']],[['1', '2']],[['2', '4','9','1']],[['3', '8']],[['2', '5','10','1']],[['4', '9']],[['2', '1']], [['5', '10']],[['6', '11','7']],[['1', '6','11','2']]]",
                        help='Meta-path Set.')
...
class DataLoaderHGNN(object):
    ...
    def __init__(self, logging, args, dataset):
        ...
        test_file = os.path.join(data_dir, 'test.txt')#协同过滤（CF）训练和测试数据
        kg_file = os.path.join(data_dir, "kg_final.txt")#知识图谱（KG）数据
    ...
    def statistic_cf(self):
        ...
        self.n_cf_train = len(self.cf_train_data[0])#计算了训练数据的数量
        self.n_cf_test = len(self.cf_test_data[0])#计算了测试数据的数量
    ...
    def construct_data(self, kg_data):
        ...
        reverse_kg_data = kg_data.copy()#创建了kg_data的一个副本
        reverse_kg_data = reverse_kg_data.rename({'h': 't', 't': 'h'}, axis='columns')#名字互换
        reverse_kg_data['r'] += n_relations
        kg_data = pd.concat([kg_data, reverse_kg_data], axis=0, ignore_index=True, sort=False)

        # re-map user id
        kg_data['r'] += 3
        self.n_relations = max(kg_data['r'])#关系的数量
        self.n_users_entities = max(max(self.cf_train_data[0]), max(self.cf_test_data[0])) + 1#用户总数量
        self.n_entities = self.n_users_entities - self.n_users#实体的数量
        ...
        if self.args.data_name == 'yelp_data':
            ...
            node_type_list[:14284] = 0#B
            node_type_list[14284:14795] = 1#Ca
            node_type_list[14795:14842] = 2#Ci
            node_type_list[14842:14853] = 3#Co
            node_type_list[14853:] = 4#U
