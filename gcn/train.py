...
def main():
    ...
    if args.model == "MetaGraph2vec":
        config.temp_file += 'graph_rw_yelp.txt'
        config.out_emd_file += args.dataset + '_node.txt'
        mgg = MetaGraphGenerator()
        if args.dataset == "yelp":
            mgg.generate_random_five(config.temp_file, config.num_walks, config.walk_length, g_hin.node,
                                      g_hin.relation_dict)
        elif args.dataset == "Douban_movie":
            mgg.generate_random_six(config.temp_file, config.num_walks, config.walk_length, g_hin.node,
                                     g_hin.relation_dict)
